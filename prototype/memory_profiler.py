...
class CodeMap(dict):
    ...
    def trace(self, code, lineno, prev_lineno):
        ...
        self[code][lineno] = (previous_inc + (memory - prev_line_memory), max(memory, previous_memory), time.time())
...
class LineProfiler(object):
    ...
    def trace_memory_usage(self, frame, event, arg):
        ...
        self.memory_times.append((_get_memory(-1, self.code_map.backend, include_children=self.code_map.include_children,
                                              filename=frame.f_code.co_filename), time.time()))
        if event == 'call':
            self.call_stack.append((frame.f_code.co_name, time.time()))
        elif event == 'return':
                last_call = self.call_stack.pop()
                self.function_times.append((last_call[0], last_call[1], time.time()))
    ...
    for (filename, lines) in prof.code_map.items():
        ...
        for (lineno, mem) in lines:
            if mem:
                ...
                mem = u''
                inc = u''
            result.append({'lineno': lineno, 'mem': mem, 'inc': inc, 'line': all_lines[lineno - 1]})
        stream.write(json.dumps(result) + "\n")
        stream.write(json.dumps(prof.memory_times) + "\n")
        stream.write(json.dumps(prof.function_times) + "\n")
