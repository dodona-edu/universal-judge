...
fun isIsbn(isbn: String?, isbn13: Boolean = true): Boolean {
    return if (isbn13) {
        isIsbn13(isbn)
    } else {
        isIsbn10(isbn)
...
fun isIsbnCheck(isbn: String?, isbn13: Boolean): Boolean {
    if (isbn !is String) {
        return false
    }
...
fun areIsbn(code: List<String?>?, isbn13: Boolean = true): List<Boolean> {
    return code!!.asSequence()
            .map { c -> isIsbnCheck(c, isbn13) }
            .toList()
}
