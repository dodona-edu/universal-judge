...
class Tile:
    ...
    def __init__(self, id, grid):
        ...
        if isinstance(grid, str):
            ...
            grid = [list(grid[row * size:(row + 1) * size]) for row in range(size)]
        self.grid = grid
...
class Puzzle:
    ...
    def solve(self):
        ...
        # create dictionary that maps borders to sets of tiles with that border
        borders = {}
        ...
        def find_next_position():
            ...
            best_position = None
            best_values = None
            for r, c in itertools.product(tuple(range(self.size)), repeat=2):
                if self.grid[r][c] is None:
                    occupied, free = 0, 0
                    for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                        if 0 <= r + dr < self.size and 0 <= c + dc < self.size:
                            if self.grid[r + dr][c + dc] is None:
                                free += 1
                            else:
                                occupied += 1
        ...
        def place_remaining_tiles(position=0):
            ...
            for index, tile_id in enumerate(remaining_tiles):
