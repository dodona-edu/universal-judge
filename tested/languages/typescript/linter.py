...
def run_eslint(
    ...
    if path := language_options.get("eslint_config", None):
        ...
        config_path = config.judge / "tested/languages/typescript/eslintrc.yml"
    ...
    if execution_results.timeout or execution_results.memory:
        return [
            (
                get_i18n_string("languages.typescript.linter.timeout")
                if execution_results.timeout
                else get_i18n_string("languages.typescript.linter.memory")
            )
        ], []
    ...
    try:
        ...
        return [
            get_i18n_string("languages.typescript.linter.output"),
            ExtendedMessage(
