...
from features import FeatureSet, combine_features, Constructs, NOTHING, WithFeatures
from testplan.ast import Expression, Statement
from testplan.channels import TextData, NoContentChannel, TextOutputChannel, \
    SpecialOutputChannel, IgnoredOutputChannel, FileOutputChannel, \
...
_FileOutput = Union[FileOutputChannel, IgnoredOutputChannel]
_ExceptionOutput = Union[ExceptionOutputChannel, SpecialOutputChannel]
_ValueOutput = Union[ValueOutputChannel, IgnoredOutputChannel]
...
class BaseOutput(WithFeatures):
    ...
    stdout: _TextOutput = NoContentChannel.NONE
    stderr: _TextOutput = NoContentChannel.NONE
    file: _FileOutput = IgnoredOutputChannel.IGNORED

    exception: _ExceptionOutput = NoContentChannel.NONE
    result: _ValueOutput = IgnoredOutputChannel.IGNORED
...
class Testcase(WithFeatures):
    ...
    def no_return_with_assignment(cls, values):
        ...
        if isinstance(input_, StatementInput) and not (
                output_.result == IgnoredOutputChannel.IGNORED
                or output_.result == NoContentChannel.NONE
        ):
...
class ContextInput(WithFeatures):
    ...
    stdin: Union[TextData, NoContentChannel] = NoContentChannel.NONE
    arguments: List[str] = field(default_factory=list)
    main_call: bool = False

    def get_as_string(self, working_directory):
        if self.stdin == NoContentChannel.NONE:
            return None
        else:
