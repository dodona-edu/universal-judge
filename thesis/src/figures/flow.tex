...
\begin{document}
    ...
    \begin{tikzpicture}
        ...
        \node[io] at (2.5,-1) (exercise) {Oefening};
        \node[process,minimum width=2cm] at (7.25,-1) (check) {Controle testplan};
        \node[io] at (12,-1) (dodonaIn) {Dodona};

        \draw[arrow] (exercise) -- (check);
        ...
        \draw[arrow] (check.south) -- (check|-plan.north);
        ...
        \draw[arrow] (11.5,-10) -- (11.5,-10-|coa.west);

        \draw[arrow] (dodonaIn.south) -- (dodonaIn|-config.north);
        \draw[arrow] (config) -- (check.east);
