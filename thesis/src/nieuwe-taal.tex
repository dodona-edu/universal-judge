...
\begin{itemize}
    \item Bestaande configuraties: \url{https://github.com/dodona-edu/universal-judge/tree/master/judge/src/tested/languages}
...
Voor we beginnen aan de configuratie, overlopen we kort welke functionaliteit we willen ondersteunen: welke functionaliteit uit C kunnen we aanbieden in \tested{} en welke functionaliteit uit \tested{} kunnen we implementeren in C?
...
\inputminted[firstline=19,lastline=29,gobble=2]{js}{sources/c-config.json}
...
\begin{description}
    ...
    \item[\texttt{named\_arguments}] Of benoemde argumenten ondersteund worden.
    Dit betekent dat de argumenten voor een functie niet enkel positioneel, maar ook op naam meegegeven kunnen worden.
    \item[\texttt{default\_parameters}] Of de programmeertaal standaardwaarden voor parameters ondersteunt, wat betekent dat ze kunnen weggelaten worden. 
\end{description}

Dan moeten we nu de ondersteuning voor de gegevenstypes vastleggen:

\inputminted[firstline=30,lastline=56,gobble=2]{js}{sources/c-config.json}
...
Hierin moet een klasse komen die van \texttt{Language} overerft:
...
Hiervoor volstaat het om de programmeertaal en de bijhorende configuratieklasse toe te voegen aan het bestand \texttt{judge/src/tested/languages/\_\_init\_\_.py}, in de dictionary \texttt{LANGUAGES}:
...
\begin{itemize}
    ...
    In sommige programmeertalen (zoals Java) moeten deze expliciet meegegeven worden, terwijl dat in andere programmeertalen (zoals C) juist niet mag.
...
Op verschillende plaatsen wordt verwezen naar andere onderdelen van de masterproef: deze kunnen nuttig zijn om het hoe en waarom te beantwoorden, maar zijn niet strikt nodig voor het implementeren van een programmeertaal.
...
We willen opmerken dat de focus in deze masterproef vooral lag op de interface naar de oefeningen toe (dus het formaat van het testplan en het serialisatieformaat).
...
Een stabiele interface tussen de programmeertalen enerzijds en de kern van \tested{} anderzijds was geen expliciet doel in deze masterproef.
...
\begin{itemize}
    \item Python
    \item Java
    \item Haskell (met \acronym{GHC}, al is er een variant die werkt met \texttt{runhaskell})
