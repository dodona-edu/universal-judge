...
Hiervoor denken we aan een \acronym{DSL} (\english{domain specific language}), maar dit valt buiten de thesis en wordt verder besproken in \cref{subsec:handgeschreven-testplannen-zijn-omslachtig}.
...
\Cref{lst:type-schema} bevat onder andere de structuur van een waarde in het serialisatieformaat, in een vereenvoudigde versie van \acronym{JSON} Schema.
...
Het doel van een formeel typesysteem is het beschrijven van welke categorieën data een constructie (een variabele, parameter, enz.) kan zijn of meekrijgen.
Dit is in tegenstelling tot \tested{}, waar concrete gegevens beschreven worden.
...
De testcode is hiervoor verantwoordelijk, en schrijft een \english{separator} naar alle uitvoerkanalen voor elk testgeval, zoals te zien is in \cref{lst:uitvoer}.
...
\Cref{lst:programmed-evaluator-python} toont hoe de definitie van het testplan en het serialisatieformaat er uitziet.
In \cref{lst:programmed-evaluator-json} staat dan de \acronym{JSON}-variant van dezelfde component: het moge duidelijk zijn dat de omzetting van Python naar \acronym{JSON} voor de hand ligt.
...
\begin{listing}
    \caption{
        ...
        Hier wordt een geprogrammeerde evaluator voorgesteld in Python-code.
    }
    \label{lst:programmed-evaluator-python}
    \inputminted{python}{code/programmed-testplan.py}
\end{listing}

\begin{listing}
    \caption{Hoe een geprogrammeerde evaluator er uitziet in \acronym{JSON}.}
    \label{lst:programmed-evaluator-json}
    \inputminted{json}{code/programmed-testplan.json}
\end{listing}