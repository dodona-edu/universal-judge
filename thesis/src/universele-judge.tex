...
\begin{listing}
    \inputminted{java}{code/testplan-specific.json}
...
Zoals reeds vermeld in onder andere \cref{subsec:testcode-uitvoeren}, wordt de testcode voor elke context afzonderlijk uitgevoerd.
...
Deze subparagraaf beschrijft de evaluatie van de implementatie van TESTed vanuit het perspectief van de performantie.

\subsection{Jupyter-kernels}\label{subsec:jupyter-kernels}

Het eerste prototype van TESTed gebruikte Jupyter-kernels voor het uitvoeren van de code.
Jupyter-kernels zijn de achterliggende technologie bij de Jupyter Notebooks \autocite{jupyter2016}.
De werking van een Jupyter-kernel kan als volgt samengevat worden: een Jupyter-kernel is een lokale server, die code kan uitvoeren en de resultaten van die uitvoer teruggeeft.
...
\begin{itemize}
    ...
    Hierdoor is het niet nodig voor elke taal veel implementatiewerk te verrichten: aangezien het protocol voor Jupyter-kernels programmeertaalonafhankelijk is, kunnen alle bestaande kernels gebruikt worden.
    ...
    \item Eerde werk \autocite{petegem2018}, dat gebruik maakt van Jupyter-kernels voor een gelijkaardig doel, rapporteert geen problemen.
\end{itemize}

Omdat contexten onafhankelijk van elkaar zijn, moet de gebruikte Jupyter-kernel gestopt en opnieuw gestart worden tussen elke context.
Dit brengt een onaanvaardbare performantiekost met zich mee, daar de meeste oefeningen niet computationeel intensief zijn.
Het uitvoeren van eenvoudige oefeningen, zoals de Lotto-oefening, duurde als snel meerdere minuten.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Bij elke context wordt voor de testcode uit te voeren een kernel uit de pool gehaald.
    ...
    \end{enumerate}
    Het idee achter deze werkwijze is dat door op een andere draad de kernels te herstarten, er altijd een kernel klaar staat om de testcode uit te voeren, en er dus niet gewacht moet worden op het opnieuw opstarten van die kernels.
    ...
    De Python-kernel (IPython) heeft bijvoorbeeld een magisch commando \texttt{\%reset}, dat alle toestand van de kernel reset.
    Het probleem is echter dat slechts weinig kernels een gelijkaardig commando hebben.
\end{itemize}
...
Zo printen de Java-kernel en Julia-kernel altijd uitvoer naar de standaarduitvoerstroom bij het opstarten, wat in bepaalde gevallen problemen gaf bij het verzamelen van de resultaten van de uitvoer van de testcode.

\subsection{Sjablonen}\label{subsec:sjablonen}

De keuze viel om verder te gaan met het systeem van sjablonen (zie ook \cref{subsec:testcode-genereren}).
Hierbij wordt de code gegenereerd en vervolgens uitgevoerd via de commandoregel.

Voordelen tegenover de Jupyter-kernels zijn:
\begin{itemize}
    ...
    \item Het is minder werk om de sjablonen voor TESTed te implementeren dan een nieuwe Jupyter-kernel te implementeren.
    ...
    \item Herbruikbaar, in die zin dat het genereren van code in een programmeertaal op basis van de programmeertaalonafhankelijke specificatie in het testplan sowieso nodig is.
\end{itemize}

Het mes snijdt aan twee kanten, dus zijn er ook nadelen:
\begin{itemize}
    \item Het gebruik van de sjablonen laat minder dynamische uitvoering van testcode toe.
    Daar de code eerst gegenereerd moet worden, moet vóór het uitvoeren bepaald worden welke code uitgevoerd zal moeten worden.
    \item Er moet meer zelf geïmplementeerd worden: het ecosysteem van Jupyter is redelijk groot, en er bestaan kernels voor veel talen (of in elk geval een begin van een kernel, waarop verder gewerkt zou kunnen worden)\footnote{Zie deze pagina voor een lijst van kernels: \url{https://github.com/jupyter/jupyter/wiki/Jupyter-kernels}.}.
    \item Het uitvoeren op de commandoregel is wel trager indien er geen reset moet gebeuren tussen de verschillende uitvoeringen (hierover later meer).
\end{itemize}
...
Bij een testplan met bijvoorbeeld 50 contexten vertaalt dit zich in 50 keer de compiler \texttt{javac} uitvoeren en ook 50 keer het uitvoeren van de code zelf met \texttt{java}.

In een tweede iteratie is een "partiële compilatie" geïmplementeerd.
Het idee hier is dat er veel code is die hetzelfde blijft voor elke context (denk aan de ingediende oplossing en hulpbestanden van TESTed).
In de partiële compilatie worden bij talen die dat ondersteunen, deze zaken eerst gecompileerd.
Bij het beoordelen van een context werd dan enkel de testcode specifiek voor te beoordelen context gecompileerd.

In een derde iteratie is de partiële compilatie uitgebreid naar de batchcompilatie.
Hoewel dit een grote winst voor de uitvoeringstijd betekende, heeft deze modus wel een ander groot nadeel (zoals ook al vermeld in \cref{subsec:testcode-genereren}): bij statische talen zorgt een compilatiefout in één context ervoor dat geen enkele context beoordeeld kan worden, daar de compilatiefout ervoor zorgt dat er geen code gegenereerd wordt.
...
De laatste en huidige iteratie bevat de mogelijkheid om beide compilatiemanieren te gebruiken, waarbij TESTed terugvalt op de contextcompilatie indien er iets misgaat bij de batchcompilatie.
...
In deze tabel is de tijd gemeten in seconden om een (juiste) ingediende oplossing voor twee oefeningen te beoordelen: de voorbeeldoefening Lotto en een eenvoudigere oefening Echo.
...
Elke beoordeling is uitgevoerd met contextcompilatie, partiële compilatie en batchcompilatie en met en zonder parallelle uitvoering.

Zoals verwacht levert de batchcompilatie het meeste tijdswinst op bij Java: daar heeft de compilatiestap ook een veel groter aandeel in de uitvoeringstijd.
By Python is de compilatie veel sneller en ook veel minder belangrijk (zelfs optioneel, de stap wordt enkel uitgevoerd om bepaalde syntaxisfouten vroeg op te vangen en als optimalisatie).
