...
\begin{enumerate}
    \item TESTed zelf is geschreven in Python: in de middelste omgeving staat de programmeertaal dus vast.
...
\begin{description}
    ...
    \item[\texttt{expression}] Vertaalt een expressie naar testcode.
\end{description}

Sjablonen kunnen zelf ook andere sjablonen gebruiken: zo gebruikt het contextsjabloon ook de sjablonen voor statements en expressies.
...
Hoewel de exacte implementatie verschilt van programmeertaal tot programmeertaal, komt het neer op het implementeren van een functie met als naam \texttt{evaluate}.
...
De \texttt{evaluate}-functie moet één oproep doen naar de functie \texttt{evaluated()}.
Deze laatste functie is een hulpfunctie die TESTed beschikbaar maakt en heeft vier argumenten:
...
Deze functie moet één oproep doen naar de functie \texttt{evaluated()}, waarvan de documentatie in fragment X gegeven is.
