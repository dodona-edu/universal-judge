...
\Cref{fig:universal-judge} toont het architecturaal ontwerp van \tested{}.
...
\begin{enumerate}
    ...
    \item De ingediende oplossing wordt uitgevoerd in de \term{uitvoeringsomgeving}, waar de programmeertaal overeenkomt met de programmeertaal van de ingediende oplossing.
...
De rest van dit hoofdstuk bespreekt in detail alle onderdelen van en stappen die gebeuren bij de beoordeling van een ingediende oplossing.
...
\begin{enumerate}
    ...
    \begin{description}
        ...
        We noemen voor het gemak de uitvoer van een compilatiestap altijd een \english{executable}, ook al gaat het technisch niet altijd om een uitvoerbaar bestand.
    ...
    \begin{description}
        \item[Programmeertaalspecifieke evaluatie (\acronym{SE})]
        ...
        \item[Geprogrammeerde evaluatie (\acronym{PE})]
        ...
        \item[Generieke evaluatie (\acronym{GE})]
...
\begin{figure}
    ...
    \caption{
        ...
        De afkortingen \acronym{PE}, \acronym{GE} en \acronym{SE} staan respectievelijk voor geprogrammeerde evaluatie, generieke evaluatie en (programmeertaal)specifieke evaluatie.
    }
...
Centraal in deze invoer is het \term{testplan} voor een oefening, een specificatie die op een programmeertaalonafhankelijke manier beschrijft hoe een oplossing moet beoordeeld worden.
...
\begin{description}
    ...
    Hierbij wordt de nadruk gelegd op de onafhankelijkheid: elke context wordt in een nieuw proces en nieuwe map (in het bestandssysteem) uitgevoerd, zodat de kans op het delen van informatie kleiner is.
    ...
    Een testgeval bestaat uit een invoer en een aantal tests.
    ...
    \begin{description}
        ...
        Als invoer voor dit testgeval kunnen enkel de standaardinvoerstroom, de commandoargumenten en de bestanden in de \english{workdir} meegegeven worden.
        ...
        De invoer is dan ook uitgebreider: het kan gaan om de standaardinvoerstroom, functieoproepen en assignments.
        Een functieoproep of assignment is verplicht (zonder functieoproep of assignment is er geen code die een resultaat zou kunnen produceren).
    \end{description}
...
Bij de keuze van een formaat voor het testplan (\acronym{JSON}, \acronym{XML}, \ldots), zijn vooraf enkele vereisten geformuleerd waaraan het gekozen formaat moet voldoen.
Het moet:
...
Niet alleen voldoet \acronym{JSON} aan de vooropgestelde voorwaarden, het wordt ook door veel programmeertalen ondersteund.

Toch zijn er ook enkele nadelen aan het gebruik van \acronym{JSON}.
Zo is \acronym{JSON} geen beknopt of compact formaat om met de hand te schrijven.
Een oplossing hiervoor gebruikt de eigenschap dat veel programmeertalen \acronym{JSON} kunnen produceren: andere programma's kunnen desgewenst het testplan in het \acronym{JSON}-formaat genereren, waardoor het niet met de hand geschreven moet worden.
Hiervoor denken we aan een \acronym{DSL} (\english{domain specific language}), maar dit valt buiten de masterproef en wordt verder besproken in \cref{subsec:handgeschreven-testplannen-zijn-omslachtig}.
...
Bij de beschrijving van de structuur van een testplan wordt gewag gemaakt van returnwaarden en assignments.
...
Een voor de hand liggende oplossing is om ook hier \acronym{JSON} te gebruiken, en zelf in \acronym{JSON} een structuur op te stellen voor de waarden.
...
\begin{itemize}
    ...
    \item Sommige formaten hebben een ander doel, wat zich uit in functionaliteit die wij niet nodig hebben, met als koploper \acronym{OPC} Unified Architecture, waarvan de specificatie ongeveer 1250 pagina's beslaat.
    ...
    Sommige dezer formaten ondersteunen weliswaar veel programmeertalen, maar we willen niet dat het serialisatieformaat een beperkende factor wordt in welke programmeertalen door \tested{} ondersteund worden.
    Het mag niet de bedoeling zijn dat het implementeren van het serialisatieformaat het meeste tijd in beslag neemt.
\end{itemize}

Voor alle bestaande formaten die geen variant van \acronym{JSON} zijn, is er nog een bijkomend nadeel: het inbedden in het \acronym{JSON}-testplan wordt moeilijker.
...
\acronym{JSON} is slechts een formaat en geeft geen semantische betekenis aan \acronym{JSON}-elementen.
...
\begin{itemize}
    \item Het encoderen van waarden. (\textit{Hoe zet ik deze lijst getallen om naar \acronym{JSON}?})
...
Het encoderen van waarden slaat op het voorstellen van waarden in \acronym{JSON}.
\acronym{JSON} heeft slechts een beperkt aantal gegevenstypes, dus worden alle waarden voorgesteld als een van deze types.
...
\Cref{lst:type-schema} bevat onder andere de structuur van een waarde (een \texttt{Literal}) in het serialisatieformaat, in een vereenvoudigde versie van \acronym{JSON} Schema.
...
% Necessary to prevent the below being split.
\clearpage
...
\begin{itemize}
    ...
    \item zelf waarden te schrijven (letterlijke modus).
    Dit gaat om serialisatie vanuit het testplan naar de uitvoeringsomgeving.
    Een voorbeeld is het opnemen van functieargumenten in het testplan: deze argumenten worden tijdens de serialisatie omgezet naar echte code.
\end{itemize}
...
Tot slot heeft het schrijven van waarden bijkomende vereisten: als functieargument zijn waarden alleen niet voldoende, ook variabelen en andere functieoproepen moeten gebruikt kunnen worden.
...
\begin{enumerate}
    ...
    \begin{description}
        ...
        \item[\texttt{set}] Een wiskundige verzameling: de volgorde van de elementen is niet belangrijk en dubbele elementen zijn niet toegelaten.
...
Zo bevat specificeert het testplan niet "een functie met returntype \texttt{int64}", maar wel "een functieoproep met als verwachte returnwaarde 16, wier gegevenstype \texttt{int64} is".
De enige uitzondering is het gegevenstype van variabelen aanduiden, maar zelfs daar gaat het om het variabelen met een concrete expressie als waarde.
...
In \cref{lst:type-schema} staat onder andere ook de structuur van een expressie en een functieoproep in een vereenvoudigde versie van \acronym{JSON} Schema.
...
\begin{enumerate}
    ...
    \begin{description}
        ...
        Dit is een lijst van ofwel expressies ofwel \english{named arguments}.
        Een named argument bestaat uit een naam en een expressie.
    \end{description}
\end{enumerate}

In het serialisatieformaat is een statement ofwel een \english{assignment}, ofwel een expressie.
Een expressie is zoals reeds besproken.
Een assignment kent een naam toe aan de resulterende waarde van een expressie.
\Cref{lst:type-schema} toont ook het vereenvoudigde \acronym{JSON} Schema van een statement (en dus van een assignment, daar er maar één soort statement bestaat).
...
\Cref{lst:advanced-expressions} bevat een voorbeeld van geavanceerde constructies die voorgesteld kunnen worden in het testplan.
...
In het stappenplan uit \cref{sec:stappenplan-van-een-beoordeling} is al vermeld dat vóór een beoordeling start, een controle plaatsvindt om zeker te zijn dat het testplan uitgevoerd kan worden in de programmeertaal van de ingediende oplossing.
...
\begin{itemize}
    ...
    Een voorbeeld van een functie met argumenten met heterogene gegevenstypes komt bijvoorbeeld uit de \acronym{ISBN}-oefening (deze oefening wordt besproken in \cref{sec:oefening-isbn}):
...
False
    \end{minted}
    In programmeertalen als Python en Java kan deze functie geïmplementeerd worden, maar in programmeertalen als C is dat veel moeilijker.
\end{itemize}
...
Tot slot moet ook het serialisatieformaat geïmplementeerd worden in de taal.
...
Elk soort gedrag of neveneffect wordt een \term{uitvoerkanaal} genoemd.
...
De reden dat deze niet naar een andere \term{file descriptor} geschreven worden is eenvoudig: niet alle programmeertalen (zoals Java) ondersteunen het openen van bijkomende file descriptors.
...
De testcode is hiervoor verantwoordelijk, en schrijft een \english{separator} naar alle uitvoerkanalen voor elk testgeval.
...
Als \english{proof of concept} zijn drie eenvoudige evaluatiemethoden ingebouwd in \tested{}, die hieronder besproken worden.
...
\begin{description}
    ...
    Bij het vergelijken met de verwachte waarde zal de functie \mintinline{python}{math.isclose()}\footnote{Documentatie is beschikbaar op: \url{https://docs.python.org/3/library/math.html\#math.isclose}} uit de standaardbibliotheek van Python gebruikt worden.
...
\Cref{lst:testplan-text} toont een fragment uit een testplan: de specificatie van \texttt{stdout} van een testgeval waarbij de tekstevaluatie gebruikt wordt.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Specifieert het testplan een uitgebreid type, dan zal het uitgebreid type gebruikt worden voor programmeertalen die dat type ondersteunen, terwijl voor andere programmeertalen het basistype gebruikt zal worden.
        Stel dat het testplan bijvoorbeeld een waarde met als gegevenstype \texttt{tuple} heeft.
        In Python en Haskell (twee programmeertalen die dat gegevenstype ondersteunen) zullen enkel \texttt{tuple}s goedgekeurd worden.
        Voor andere programmeertalen, zoals Java, worden alle gegevenstypes goedgekeurd die herleidbaar zijn tot het basistype.
    ...
    Eén uitzondering is zwevendekommagetallen, waarvoor \mintinline{python}{math.isclose()} gebruikt wordt.
\end{enumerate}
...
\tested{} vereist een \acronym{JSON}-object met vier velden:
...
\begin{itemize}
    ...
    \item Het uitvoeren op de commandoregel is bij veel programmeertalen trager indien er geen \english{cold start} moet gebeuren tussen de contexten.
...
De gemakkelijkste oplossing is echter de annotatie uitbreiden met ondersteuning voor een \acronym{URL}: bij veel linters kan met de identificatiecode van het bericht een \acronym{URL} geconstrueerd worden waarop meer informatie staat.
...
De feedback bij een verkeerde oplossing is in veel gevallen zelfs belangrijker dan de feedback bij een juiste oplossing: het is namelijk de bedoeling dat als studenten een verkeerde oplossing indienen, de feedback ze terug op weg kan helpen om hun oplossing te verbeteren.
...
Een van die oefeningen is de Python-versie van de \emph{\acronym{ISBN}}-oefening.
De opgave van deze oefening is opgenomen als onderdeel van \cref{sec:oefening-isbn}.
Om een idee te krijgen van hoe \tested{} in de praktijk werkt, is besloten om tijdens een van de weken de \emph{\acronym{ISBN}}-oefening aan te vullen door een oefening met dezelfde opgave, maar die beoordeeld wordt door \tested{} in plaats van de Python-judge.
...
\begin{itemize}
    ...
    \item \tested{} heeft geen verwerking van stack traces.
...
\begin{description}
    ...
    De stack traces bij deze uitvoer bevatten wel referenties naar code die \tested{} gegenereerd heeft.
...
\begin{enumerate}
    \item Mako voor de sjablonen.
    \item Pydantic voor het definiëren van het testplan en het serialisatieformaat.
\end{enumerate}
...
Na de keuze voor \acronym{JSON} als testplan en serialisatieformaat (\cref{subsec:het-testplan,subsec:dataserialisatie}) is er beslist om de implementatie in Python de gezaghebbende definitie voor het formaat te maken.
Oorspronkelijk zijn het testplan en het serialisatieformaat opgesteld met \texttt{dataclasses} uit Python.
Dit bleek niet voldoende: er waren betere validaties nodig en het bleek handig om het formaat van het testplan en het serialisatieformaat te kunnen exporteren als \acronym{JSON} Schema.

Hiervoor gebruiken we nu Pydantic\footnote{Zie \url{https://pydantic-docs.helpmanual.io/}}.
Dit package laat ons toe om nog steeds \texttt{dataclasses} te gebruiken, maar toch validaties te kunnen toevoegen en de structuur te kunnen exporteren naar \acronym{JSON} Schema.
\Cref{lst:function-python} toont hoe de definitie van een functieoproep er in Python-code uitziet.
Deze klasse wordt letterlijk vertaald naar \acronym{JSON}, zoals duidelijk is in bijvoorbeeld het testplan in \cref{subsec:oefening-echofunctie-testplan}.

Het \acronym{JSON} Schema voor respectievelijk het testplan en het serialisatieformaat kunnen zo automatisch verkregen worden:
