...
class InOutTester(Tester):
    ...
    def execute_statement(self, code, input_, context: JupyterContext, timeout, memory_limit) -> ExecutionResult:
        """Execute code."""
        ...
        for error in errors_:
            if error['ename'] == 'TimeoutError':
                ...
                messages.append(ExtendedMessage(description='<pre>' + '<br>'.join(coloured) + '</pre>', format='html'))
    ...
    def evaluate_code(self, code, input_, expected, context: JupyterContext) -> po.CloseTest:
        ...
        r = self.execute_statement(code, input_, context,
                                   timeout=self.config.time_limit,
                                   memory_limit=self.config.memory_limit)

        # TODO: custom evaluator and such
        ...
        if r.errors:
            ...
            stderr = "\n".join(r.stderr)
            if stderr:
                r.messages.append(ExtendedMessage(description=stderr, format='text'))
        else:
            # Evaluate solution
            actual = "\n".join(r.stdout)
            evaluator = SimpleStringComparator()
            if evaluator.compare(expected, actual):
                status = po.Status.CORRECT
            else:
                status = po.Status.WRONG

            # Everything was successful
            r = self.execute_statement("%reset", None, context)
